...
from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, autopad, Conv2
...
class GPSABlock(nn.Module):
    ...
    def __init__(self, c, attn_ratio=0.5, num_heads=4, shortcut=True) -> None:
        ...
        self.attn = Attention(c, attn_ratio=attn_ratio, num_heads=num_heads)
        self.ffn = nn.Sequential(GhostConv(c, c * 2, 1), GhostConv(c * 2, c, 1, act=False))
        self.add = shortcut
...
class GPSA(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, e=0.5):
        ...
        self.cv2 = GhostConv(2 * self.c, c1, 1)

        self.m = nn.Sequential(*(GPSABlock(self.c, attn_ratio=0.5, num_heads=self.c // 64) for _ in range(n)))
...
class OurBlock(nn.Module):
    def __init__(self, c1, c2, n=1, scale=0.5, e=0.5):
        ...
        self.m = nn.ModuleList(SELayerV2(self.mid) for _ in range(n - 1))
        self.cv4 = DEConv(self.mid)
...
class Parameter_Concat(nn.Module):
    def __init__(self, dimension=1, num_inputs=3):
        super(Parameter_Concat, self).__init__()
        self.d = dimension
    ...
    def forward(self, x):
        if len(x) != self.num_inputs:
            raise ValueError(f"Expected {self.num_inputs} inputs, but got {len(x)}.")
            
        weight = self.w / (torch.sum(self.w, dim=0) + self.epsilon)
        weighted_inputs = [weight[i] * x[i] for i in range(self.num_inputs)]
        
        return torch.cat(weighted_inputs, self.d)
